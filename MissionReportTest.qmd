...
depthDF <- gliderdf %>%
  ...
  mutate(gDepth = max(osg_i_depth, na.rm = TRUE),
         ...
         battDelta = max(m_battery, na.rm = TRUE) - min(m_battery, na.rm = TRUE)) %>%
  filter(!is.na(gDepth)) %>%
  ungroup() %>%
...
#| echo: false
  ...
  (batt <- ggplot() +
    geom_point(data = dailydf, 
               size = 2, aes(x = day, y = m_battery), color = "red") +
  ...
  (roll <- ggplot() +
    geom_point(data = dailydf, 
               size = 2, aes(x = day, y = ((m_roll*180)/pi)), color = "darkgreen") +
  ...
  (vacta <- ggplot() +
    geom_point(data = dailydf, 
               size = 2, aes(x = day, y = m_vacuum), color = "darkred") +
    labs(title = "Daily vacuum",
         x = "Date", 
         y = "Vacuum (inHg") +
    theme_bw())
